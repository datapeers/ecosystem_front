...
export class StorageService {
  ...
  uploadFile(filePath: StoragePaths, file: File): Observable<HttpEvent<HttpEventType>> {
    // Get a presigned URL for uploading the file
    const headers = new HttpHeaders();
    headers.set("Content-type", "application/json");
    const request = this.http.post<{ url: string} >(this.apiUrl, { name: `${filePath}/${file.name}` }, { headers: headers });
    return request.pipe(
  ...
  deleteFile(filePath: string, fileName: string): Observable<HttpEvent<HttpEventType>> {
    // Get a presigned URL for uploading the file
    const headers = new HttpHeaders();
    headers.set("Content-type", "application/json");
    const request = this.http.post<{ url: string} >(this.apiUrl, { name: `${filePath}/${fileName}` }, { headers: headers });
    return request.pipe(
      mergeMap(({ url: presignedUrl}) => {
        // Use the presigned URL to upload the file to S3
